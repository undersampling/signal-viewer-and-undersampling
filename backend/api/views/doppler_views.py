...
# # # # Optional model/config paths via Django settings, env vars, or defaults under backend/models
# # # _default_models_dir = os.path.join(getattr(settings, 'BASE_DIR', os.path.dirname(os.path.dirname(os.path.dirname(__file__)))), 'backend', 'models')
# # # MODEL_PATH = (
# # #     getattr(settings, 'DOPPLER_MODEL_PATH', None)
# # #     or os.environ.get('DOPPLER_MODEL_PATH', None)
# # #     or os.path.join(_default_models_dir, 'doppler_regressor_3_outputs.keras')
# # # )
# # # CONFIG_PATH = (
# # #     getattr(settings, 'DOPPLER_CONFIG_PATH', None)
# # #     or os.environ.get('DOPPLER_CONFIG_PATH', None)
# # #     or os.path.join(_default_models_dir, 'spectrogram_width.npy')
# # # )
# # # REG_MODEL = None
...
# #         store = {'sr': sr, 'samples': doppler_out.tolist(), 'duration': len(doppler_out) / sr}
# #         fig = _make_waveform(store)
...
# #         return Response({'store': store, 'src': src, 'waveform': fig, 'status': status_msg, 'observed': freq_msg})
# #     except Exception as e:
# #         return Response({'error': str(e)}, status=status.HTTP_400_BAD_REQUEST)
...
# @api_view(['POST'])
# @parser_classes([JSONParser])
# def waveform_chunk(request):
#     """Return waveform chunk for streaming playback (for DisplayAudio)"""
#     try:
#         file_id = request.data.get('file_id')
#         position = float(request.data.get('position', 0))
        
#         if not file_id or file_id not in AUDIO_STORAGE:
...
#         # Return chunk around position
#         window_size = 0.5  # seconds
#         start_time = max(0, position - window_size / 2)
#         end_time = min(duration, position + window_size / 2)
        
#         start_idx = int(start_time * sr)
#         end_idx = int(end_time * sr)
        
#         chunk_samples = samples[start_idx:end_idx]
#         chunk_time = np.linspace(start_time, end_time, len(chunk_samples))
        
#         return Response({
#             'time': chunk_time.tolist(),
#             'amplitude': chunk_samples.tolist(),
#             'new_position': min(position + 0.1, duration),
