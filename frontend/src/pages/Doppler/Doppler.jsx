...
//       {originalAnalysis && dopplerAnalysis && (
...
//               <div className="comparison-side">
//                 <h3> Resample Doppler-Shifted Audio </h3>
//                 <DisplayAudio
//                   analysis={dopplerAnalysis}
//                   audioSrc={dopplerSrc}
//                   setError={setError}
...
const Doppler = () => {
  ...
  const [dopplerDownsampledAnalysis, setDopplerDownsampledAnalysis] = useState(null);
  ...
  const onGenerate = async () => {
    ...
    try {
      ...
      delete dopplerAnalysisData.prediction;
      setDopplerAnalysis(dopplerAnalysisData);

      // Capture original sample rate
      if (data.waveform && data.waveform.sr) {
        setDopplerOriginalRate(data.waveform.sr);
      } else if (data.freq_time_data && data.freq_time_data.sr) {
        setDopplerOriginalRate(data.freq_time_data.sr);
      } else {
        setDopplerOriginalRate(48000);
      }
  ...
  const analyzeDopplerDownsampledAudio = useCallback(async (audioDataUri) => {
    try {
      const response = await fetch(audioDataUri);
      const blob = await response.blob();
      const downsampledFile = new File([blob], `downsampled_doppler_${resampleRate}.wav`, { type: "audio/wav" });
  ...
  const handleDopplerDownsample = useCallback(async () => {
    ...
    try {
      ...
      const dopplerFile = new File([audioBlob], "doppler_audio.wav", { type: audioBlob.type });

      const response = await apiService.downsampleAudio(dopplerFile, resampleRate);
  ...
  }, [dopplerSrc, resampleRate, dopplerOriginalRate, dataURItoBlob, analyzeDopplerDownsampledAudio]);
  ...
  const toggleComparison = () => {
    ...
    setShowComparison(prev => {
      ...
      if (newShowComparison) {
        const initialRate = dopplerOriginalRate > 0 ? Math.min(16000, dopplerOriginalRate) : 16000;
    ...
    <div className="doppler-page" style={{ padding: 16 }}>
      ...
      {dopplerAnalysis && (
        <div className="results-container">
          {/* Comparison Toggle Button */}
          <div className="comparison-controls">
            <button className="btn btn-secondary" onClick={toggleComparison} disabled={isDopplerDownsampling || loading}>
          ...
          {!showComparison ? (
            ...
            <div className="comparison-container">
              <div className="comparison-side">
                <h3>Doppler Audio ({dopplerOriginalRate > 0 ? dopplerOriginalRate : 'N/A'} Hz)</h3>
                <DisplayAudio
                  analysis={dopplerAnalysis}
                  audioSrc={dopplerSrc}
                  setError={setError}
